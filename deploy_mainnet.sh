...
PROFILE=${1:-"lucky-mainnet"}
SEED=${2:-"lucky_survivor_mainnet_v2"}
...
# Safety prompt for mainnet deployment
...
if [ -z "$DEPLOYER" ] || [ "$DEPLOYER" == "null" ]; then
  echo "Error: Profile $PROFILE not found. Run: aptos init --profile $PROFILE --network mainnet"
  exit 1
fi
...
echo "Explorer: https://explorer.aptoslabs.com/account/$RESOURCE_ADDRESS?network=mainnet"
echo "========================================="

# Save addresses for reference
cat > addresses_mainnet.json <<EOF
{
  "network": "mainnet",
  "deployer": "$DEPLOYER",
